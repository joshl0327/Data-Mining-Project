...
class Stocker():
    ...
    def __init__(self, ticker, exchange='WIKI'):
        ...
        print('{} Stocker Initialized. Data covers {} to {}.'.format(self.symbol,
                                                                     pd.Timestamp(self.min_date),
                                                                     pd.Timestamp(self.max_date)))
    ...
    def handle_dates(self, start_date, end_date):
        ...
        while (not valid_start) & (not valid_end):
            ...
            if pd.Timestamp(end_date) < pd.Timestamp(start_date):
                ...
                start_date = pd.Timestamp(input('Enter a new start date: '))
                end_date= pd.Timestamp(input('Enter a new end date: '))
                valid_end = False
                valid_start = False
            
            else: 
                if pd.Timestamp(end_date) > pd.Timestamp(self.max_date):
                    print('End Date exceeds data range')
                    end_date= pd.Timestamp(input('Enter a new end date: '))
                    valid_end = False

                if pd.Timestamp(start_date) < pd.Timestamp(self.min_date):
                    print('Start Date is before date range. Will choose, ' + 
                          str(pd.Timestamp(self.min_date)) + ', the initial stock data date as Start Date')
                    #start_date = pd.Timestamp(input('Enter a new start date: '))
                    valid_start = False
    ...
    def make_df(self, start_date, end_date, df=None):
        ...
        if self.round_dates:
            ...
            if (not end_in) & (not start_in):
                trim_df = df[(df['Date'] >= pd.Timestamp(start_date)) & 
            ...
            while (not valid_start) & (not valid_end):
                ...
                if (start_date not in list(df['Date'])):
                    ...
                    start_date = pd.Timestamp(input(prompt='Enter a new start date: '))
                    
                elif (end_date not in list(df['Date'])):
                    print('End Date not in data (either out of range or not a trading day.)')
                    end_date = pd.Timestamp(input(prompt='Enter a new end date: ') )
    ...
    def plot_stock(self, start_date=None, end_date=None, stats=['Adj. Close'], plot_type='basic'):
        ...
        for i, stat in enumerate(stats):
            ...
            date_stat_min = pd.Timestamp(date_stat_min[date_stat_min.index[0]])
            date_stat_max = stock_plot[stock_plot[stat] == stat_max]['Date']
            date_stat_max = pd.Timestamp(date_stat_max[date_stat_max.index[0]])
            
            print('Maximum {} = {:.2f} on {}.'.format(stat, stat_max, date_stat_max))
            print('Minimum {} = {:.2f} on {}.'.format(stat, stat_min, date_stat_min))
            print('Current {} = {:.2f} on {}.\n'.format(stat, self.stock.loc[len(self.stock) - 1, stat], pd.Timestamp(self.max_date)))
    ...
    def buy_and_hold(self, start_date=None, end_date=None, nshares=1):
        ...
        print('{} Total buy and hold profit from {} to {} for {} shares = ${:.2f}'.format
              (self.symbol, pd.Timestamp(start_date), pd.Timestamp(end_date), nshares, total_hold_profit))
        
        # Plot the total profits 
        plt.style.use('dark_background')
        
        # Location for number of profit
        text_location = (end_date - pd.Timestamp(pd.DateOffset(months = 1)))
        ...
        plt.ylabel('Profit ($)'); plt.xlabel('Date'); plt.title('Buy and Hold Profits for {} {} to {}'.format(
                                                                self.symbol, pd.Timestamp(start_date), pd.Timestamp(end_date)))
    ...
    def changepoint_prior_analysis(self, changepoint_priors=[0.001, 0.05, 0.1, 0.2], colors=['b', 'r', 'grey', 'gold']):
        ...
        train = self.stock[(self.stock['Date'] > (max(self.stock['Date']) - pd.Timestamp(pd.DateOffset(years=self.training_years))))]
    ...
    def create_prophet_model(self, days=0, resample=False):
        ...
        stock_history = self.stock[self.stock['Date'] > pd.Timestamp(self.max_date - pd.DateOffset(years = self.training_years))]
        ...
        if days > 0:
            ...
            print('Predicted Price on {} = ${:.2f}'.format(
                pd.Timestamp(future.loc[len(future) - 1, 'ds']), future.loc[len(future) - 1, 'yhat']))
    ...
    def evaluate_prediction(self, start_date=None, end_date=None, nshares = None):
        ...
        train = self.stock[(self.stock['Date'] < start_date.date()) & 
                           (self.stock['Date'] > (start_date - pd.Timestamp(pd.DateOffset(years=self.training_years))))]
        
        # Testing data is specified in the range
        test = self.stock[(self.stock['Date'] >= pd.Timestamp(start_date)) & (self.stock['Date'] <= pd.Timestamp(end_date))]
        ...
        if not nshares:
            ...
            print('\nPrediction Range: {} to {}.'.format(pd.Timestamp(start_date),
                pd.Timestamp(end_date)))

            # Final prediction vs actual value
            print('\nPredicted price on {} = ${:.2f}.'.format(max(pd.Timestamp(future['ds'])), future.loc[len(future) - 1, 'yhat']))
            print('Actual price on    {} = ${:.2f}.\n'.format(max(pd.Timestamp(test['ds'])), test.loc[len(test) - 1, 'y']))
            ...
            plt.vlines(x=pd.Timestamp(min(test['ds'])), ymin=min(future['yhat_lower']), ymax=max(future['yhat_upper']), colors = 'r',
        ...
        # If a number of shares is specified, play the game
            ...
            print('You played the stock market in {} from {} to {} with {} shares.\n'.format(
                self.symbol, pd.Timestamp(start_date), pd.Timestamp(end_date), nshares))
            ...
            text_location = pd.Timestamp((last_date - pd.DateOffset(months = 1)))
    ...
    def changepoint_date_analysis(self, search=None):
        ...
        train = self.stock[self.stock['Date'] > pd.Timestamp((self.max_date - pd.DateOffset(years = self.training_years)))]
        model.fit(train)
        ...
        for changepoint in (changepoints):
            change_indices.append(train[train['ds'] == pd.Timestamp(changepoint)].index[0])
    ...
    def predict_future(self, days=30):
        ...
        train = self.stock[self.stock['Date'] > pd.Timestamp((max(self.stock['Date']) - pd.DateOffset(years=self.training_years)))]
        ...
        future = future[future['ds'] >= pd.Timestamp(max(self.stock['Date']))]
    ...
    def changepoint_prior_validation(self, start_date=None, end_date=None,changepoint_priors = [0.001, 0.05, 0.1, 0.2]):
        ...
        train = self.stock[(self.stock['Date'] > pd.Timestamp((start_date - pd.DateOffset(years=self.training_years)))) & 
        (self.stock['Date'] < pd.Timestamp(start_date))]
        
        # Testing data is specified by range
        test = self.stock[(self.stock['Date'] >= pd.Timestamp(start_date)) & (self.stock['Date'] <= pd.Timestamp(end_date))]

        eval_days = (pd.Timestamp(max(test['Date'])) - pd.Timestamp(min(test['Date']))).days
        
        results = pd.DataFrame(0, index = list(range(len(changepoint_priors))), 
            columns = ['cps', 'train_err', 'train_range', 'test_err', 'test_range'])

        print('\nValidation Range {} to {}.\n'.format(pd.Timestamp(min(test['Date'])),
            pd.Timestamp(max(test['Date']))))
